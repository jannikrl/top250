...
class ImdbService {
  ...
  async getMoviesFromApi() {
    ...
    if (!response.ok) {
      throw new Error('ImdbService getMoviesFromApi failed, HTTP status ${response.status}');
    }
    ...
    if (!movies) {
      throw new Error('ImdbService getMoviesFromApi failed, movies not returned');
    }
